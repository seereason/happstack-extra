...
utf8ToUnicode :: L.ByteString -> String
utf8ToUnicode s =
    -- To Do: implement this using the ByteString directly.
    decodeString (U.toString s)
    where
      decodeString ('\r' : '\n' : t) = '\n' : decodeString t
      decodeString s = decodeCharRef s
      decodeCharRef s =
          case matchRegexAll refRE s of
            Just ("", _, t, [n])
              | read n < 0x110000 -> chr (read n) : decodeString t
              | otherwise -> decodeEntityRef s
            Nothing -> decodeEntityRef s
            x -> error $ "Internal error in lookPairsUnicode: " ++ show x
      decodeEntityRef s = 
          case matchRegexAll entityRE s of
            Just ("", _, t, [w]) ->
                case Map.lookup w entityMap of
      ...
      decodeOther [] = [] 
      refRE = mkRegexWithOpts "^&#([0-9]+);" False True
      entityRE = mkRegexWithOpts "^&(.*);" False True

entityMap = Map.fromList (map (\ (name, n) -> (name, chr n)) entityPairs)

{- Build the entity list using these commands:
  { echo -n "entityTable = Map.fromList\n[";
    curl 'http://www.w3.org/TR/html5/named.html' | sed -n 's/^.*code title="">\([^;]*\);.*U+\([^ ]*\) .*$/ ("\1", 0x\2),/p';
    echo -n "]"; }
-}

-- This list is for more for decoding than encoding, it is not clear
-- whether any given browser will support all of these entities.
entityPairs =
    [("AElig", 0x000C6),
     ("AMP", 0x00026),
     ("Aacute", 0x000C1),
